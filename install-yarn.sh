#!/bin/bash
# Read the desired yarn version string from package1.json - it should be in the top level key yarnVersion
EXPECTED_YARN_VERSION=$(node -e 'const fs = require("fs"); console.log(JSON.parse(fs.readFileSync("package1.json")).yarnVersion || "");')
